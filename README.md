# Python Learning Journey | Python学习历程

Welcome to my personal repository where I document my **Python** learning journey. I welcome any suggestions or corrections from fellow learners!  
本仓库主要记录个人学习**Python**的历程，欢迎各位指正！

Windows | VS Code | Python 3.10.11

## Current Learning Progress | 当前学习进展

- **Mastering Python Basics**: I'm currently focusing on mastering Python's basic syntax.  
  **掌握Python基础语法**：我目前专注于掌握Python的基础语法。
  
- **Next Step**: After gaining proficiency in Python, I'll continue my learning path with "[Hands-on Learning of Reinforcement Learning](https://hrl.boyuai.com/)." I plan to create a new repository named **"rl_learning"** for this.  
  **下一步**：在掌握Python基础后，我将继续学习"[动手学强化学习](https://hrl.boyuai.com/)"，届时会创建一个新的仓库命名为 **"rl_learning"**。

## Learning Resources | 学习资源

- **Main Reference 1**: [Bilibili Uploader: 林粒粒呀](https://www.bilibili.com/video/BV1Jgf6YvE8e/), where I follow tutorials and concepts related to Python.  
  **主要参考 1**：[B站up主: 林粒粒呀](https://www.bilibili.com/video/BV1Jgf6YvE8e/)，我在这里跟随教程学习Python相关的概念。

- **Main Reference 2**: [Python 100 Examples](https://www.runoob.com/python/python-exercise-example1.html), where I will practice Python syntax through real-world applications, preparing for future LeetCode challenges.  
  **主要参考 2**：[菜鸟一百题](https://www.runoob.com/python/python-exercise-example1.html)，在这里，我会通过实际应用来练习Python语法，为后续刷LeetCode做准备。

## Status Update | 状态更新

I have completed all the tutorials from [Bilibili Uploader: 林粒粒呀](https://www.bilibili.com/video/BV1Jgf6YvE8e/), and now I am working on the [Python 100 Examples](https://www.runoob.com/python/python-100-examples.html). **[Continuous Update]**  
我已经学完了[林粒粒呀](https://www.bilibili.com/video/BV1Jgf6YvE8e/)的内容，接下来我会做[菜鸟100题](https://www.runoob.com/python/python-100-examples.html)【持续更新中】。

### Demo1 ~ Demo100 | 题目与求解代码

In this section, I am working through the 100 exercises from [Python 100 Examples](https://www.runoob.com/python/python-100-examples.html). Each exercise includes:   
在本部分，我正在做[菜鸟一百题](https://www.runoob.com/python/python-100-examples.html)的100个练习。每道题包括：

1. The problem statement  
   题目描述  
2. The solution code  
   求解代码
3. Some exercises will include my personal thoughts and reflections on the solution at the end  
   部分题目会在末尾引入我的个人思考和对解决方案的反思  
4. Possible multiple solutions  
   可能有多解

for instance:  
例如：  

<img width="3775" height="1092" alt="image" src="https://github.com/user-attachments/assets/be9a71c2-202a-4d0b-bdf3-85417c0f2ede" />



Feel free to explore the solutions, and I encourage others to improve or discuss the solutions with me.  
欢迎大家一起探索解决方案，并鼓励大家提供改进意见或与我讨论。

Feel free to explore the repository and share insights, or join me on this exciting journey of learning! The repository contains two main folders:  
欢迎大家浏览我的仓库、分享见解，或者和我一起踏上这段激动人心的学习之旅！仓库包含两个主要文件夹：

- `py_learning`: Contains the content from [Bilibili Uploader: 林粒粒呀](https://www.bilibili.com/video/BV1Jgf6YvE8e/)
- `python_100_examples`: Contains the exercises from Demo1 to Demo100, from [菜鸟100题](https://www.runoob.com/python/python-exercise-example1.html)  
